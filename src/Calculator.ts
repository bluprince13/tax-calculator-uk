...
const getEffectiveTaxRate = (grossIncomeAfterSalarySacrifice: number, earningsYearly: number): number => {
  const effectiveTax = grossIncomeAfterSalarySacrifice - earningsYearly
  return getAmountRounded((effectiveTax * 100) / grossIncomeAfterSalarySacrifice)
}
...
const Calculator = (grossIncome: number, options: CalculatorOptions) => {
  ...
  calculator.getTaxBreakdown = () => {
    return {
      ...
      effectiveTaxRate: getEffectiveTaxRate(getGrossIncomeAfterSalarySacrifice(), getTotalNetPayPerYear())
    }
  }
